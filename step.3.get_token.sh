...
sudo mount -t tmpfs -o size=1M,mode=0755 tmpfs /var/klovercloud.com/cache
sudo chown $(whoami) /var/klovercloud.com/cache

export JOIN=`ssh $kube_master sudo kubeadm token create --ttl 30m --print-join-command` 

export OUTPUT=`ssh $kube_master sudo kubeadm init phase upload-certs --upload-certs | tail -n 1`
